...
const galleryImageSchema = z.object({
  barberShopID: z.number().int().positive('barberShopID must be positive'),
...
export class ConfigController {
  ...
  static async getGalleryImages(_req: Request, res: Response, next: NextFunction) {
    try {
      const rows = await db.select().from(schema.GalleryImage);
      res.json(rows);
    } catch (err) {
      console.error('ConfigController.getGalleryImages error:', err);
      next(err);
    }
  }

  static async getGalleryImage(req: Request, res: Response, next: NextFunction) {
    try {
      const id = Number(req.params.id);
      if (Number.isNaN(id)) {
      ...
      if (!row) {
        return res.status(404).json({ error: 'GalleryImage not found' });
      }

      res.json(row);
    } catch (err) {
      console.error('ConfigController.getGalleryImage error:', err);
      next(err);
    }
  }

  static async createGalleryImage(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const [created] = await db
        .select()
        .from(schema.GalleryImage)
        .where(eq(schema.GalleryImage.imageID, insertId));
      ...
      console.error('ConfigController.createGalleryImage error:', err);
      next(err);
    }
  }

  static async updateGalleryImage(req: Request, res: Response, next: NextFunction) {
    try {
      const id = Number(req.params.id);
      if (Number.isNaN(id)) {
        return res.status(400).json({ error: 'Invalid imageID' });
      ...
      console.error('ConfigController.updateGalleryImage error:', err);
      next(err);
    }
  }

  static async deleteGalleryImage(req: Request, res: Response, next: NextFunction) {
    try {
      const id = Number(req.params.id);
      if (Number.isNaN(id)) {
        return res.status(400).json({ error: 'Invalid imageID' });
      ...
      await db.delete(schema.GalleryImage).where(eq(schema.GalleryImage.imageID, id)).execute();
      res.json({ success: true });
    } catch (err) {
      console.error('ConfigController.deleteGalleryImage error:', err);
      next(err);
    }
  }
